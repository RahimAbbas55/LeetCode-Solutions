...
var findKDistantIndices = function(nums, key, k) {
    let indicesArr = [] , n = nums.length , result = [] , p = 0;
    // get all the indices first
    for ( let i = 0 ; i < n ; i++ ){
        if ( nums[i] === key ){
            indicesArr.push(i)
        }
    ...
    for ( let i = 0 ; i < n ; i++ ){
        while ( p < indicesArr.length && indicesArr[p] < i - k ){
            p++;
        }
        if ( p < indicesArr.length && Math.abs(indicesArr[p] - i) <=k ){
            result.push(i);
        }
    }
   return result;
